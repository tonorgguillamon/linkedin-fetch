...
def run_app(api):
    ...
    def search_jobs():
        jobs = []
        keywords = request.form['keywords']
        jobs_fetched = api.search_jobs(keywords, listed_at=1 * 24 * 60 * 60, limit=10, offset=0)
        job_ids = get_job_ids(jobs_fetched)
        for id in job_ids:
            job_info = api.get_job(id)
            print(job_info)
            jobs.append(Job(**scrap_job(job_info)))
            #TODO: Add job to the somewhere!

        print("Number of jobs found: " + str(len(jobs)))
        return render_template('jobs_search.html', companies=None, jobs=jobs)
